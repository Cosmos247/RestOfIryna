...
- `Swift/Models/User.swift` — User model (identity, class, nickname, estate, stats, gold, last_hp_tick_at) + effective-stat computed properties. Also carries the non-persisted `transientInExpedition: Bool` flag that `RouterStore.process` refreshes on every dispatch so controllers can branch on expedition presence without a DB query.
