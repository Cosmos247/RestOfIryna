...
- `Swift/Services/` — Domain services. HungerService is pure (callers persist). EquipmentService owns the atomic slot swap and saves entries + user itself. WarehouseService transfers one unit at a time between inventory and warehouse (skips equipped rows on deposit; preflights inventory space on withdraw and returns a typed result enum). Currently: HungerService, EquipmentService, WarehouseService.
