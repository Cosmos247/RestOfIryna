...
- `Swift/Models/Enemy.swift` — code-based bestiary (Enemy + EnemyLootDrop + EnemyCatalog). 7 animals across 6 tiers (T1 = km 1-5 up to T6 = km 26-35; rabid_wolf and wild_bear bridge the deep zone with overlapping ranges). Two families: wild (🐗 boar / 🫎 moose / 🦬 buffalo / 🐻 bear — drop raw meat + hide) and rabid (🐈‍⬛ lynx / 🐺 wolf / 🐻‍❄️ bear — drop hide only because rabies-tainted meat is inedible). Full reference doc lives at `content/bestiary.md` with per-enemy stats, loot tables, and tier/family overviews. T5+ currently has only regular mobs; the dedicated boss encounter is reserved for Phase 3.5 once boss-fight mechanics are designed.
