...
- `Localizations/` — `en.json`, `uk.json` (~214 keys each). `exploration.find.<item_id>` keys carry per-item foraging flavor text used by the active-mode loot narrator.
...
**Combat (Phase 4.1 — foundation in flight):** Shared damage primitives extracted into `CombatService.applyAttack` (and the chip-damage helper for Defend). `ExplorationService.resolveAutobattle` now calls into them, so passive autobattle picks up crit / dodge / accuracy semantics for free (active CombatController will share the same odds when it lands). Migration `AddCombatFields` adds nullable `combat_enemy_id` + `combat_enemy_hp` columns on `exploration_state` so the active controller can persist a live combat across taps. **Not yet wired:** the new `StepOutcome.encounterStarted` case, the CombatController itself, locale keys for class-flavoured Attack/Defend/Flee buttons. See `TODO.md` Phase 4.1 for the remaining checklist.

**Not started:** Real combat UI (Phase 4.1 in flight — see Combat block above), artifact activation, crafting, pets, guilds, arena, market, territorial warfare, daily 2h passive budget, early-cancel button for in-flight passive, production-mode durations (flip `PassiveExpeditionService.testMode` to false when ready), content expansion (3.5 — more enemy tiers, richer event pool). Hunger drain is wired: walk-room drain + combat-round drain + per-room starvation HP tick fire inside `ExplorationService.rollStep` on every step (both active and passive simulation).
