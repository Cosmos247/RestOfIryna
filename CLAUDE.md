...
- `Swift/Controllers/` — Game screen controllers (Main, Registration, Settings, GlobalCommands, Inventory tree nav, Estate tree nav, Exploration active-mode MVP; stub for Capital)
...
- `Swift/Models/ExplorationState.swift` — Fluent model, one row per active expedition (user_id unique, stepsDeep = current km, `visited_rooms` JSON dict of km → visit count). Presence = exploring; absence = at estate. begin/current/end/recordVisit/visitCount helpers. The schema still has a dormant `returning` Bool column from an earlier 3.2 pass.
...
- `Localizations/` — `en.json`, `uk.json` (~153 keys each)
...
**Exploration (Phase 3.1 MVP + 3.2 per-room visit decay + passive estate regen landed):** Tapping 🗺 Explore enters `ExplorationController`. A dedicated reply keyboard `[🚶 Step fwd] [🔙 Step back]` / `[🎒 Bag]` replaces main nav for the expedition. Step Forward increments `stepsDeep` and fires `ExplorationService.rollStep` at the new km with the room's prior visit count; the service picks a three-tier weight table — tier 0 fresh (nothing 20 / loot 40 / encounter 30 / trip 10), tier 1 reduced (50/20/20/10), tier 2+ bare (100/0/0/0 — only .nothing / .starvationOnly fires). After the roll the room's counter is incremented, so oscillating between two rooms deplete them fast. While the player is at the estate (any non-exploration controller), `HealingService.tick` lazy-computes passive HP regen — 5% of maxHp per minute — on every interaction via `RouterStore.process`. Walk-room hunger drain + per-step starvation HP tick (when hunger == 0) fire on every step regardless of direction. Loot is added directly to inventory with a full-bag fallback. Encounters roll an enemy from `EnemyCatalog.pickFor(kmDepth:)` (3 tier-1 enemies currently) through a stub autobattle (alternating strikes, ±10% variance, hunger drained per round, safety cap 50); win → loot drops per loot table, lose → HP hits 0. Step Back at km ≥ 2 decrements + rolls at the new km using the same three-tier logic; at km ≤ 1 it ends the expedition cleanly with an "arrived home" narrative (no event roll). `/start` and stray Cancel button presses force-end without walking back (dev escape hatch). Three `.nothing` narrative variants render based on prior visit count: `exploration.outcome.nothing` (fresh), `.revisited` (thinned), `.bare` (room picked clean). Bag button shows a scoped inline view of food+potions with one-tap Eat/Use that refreshes in place. Death wipes every non-equipped `InventoryEntry` row, respawns at HP=1 (hunger preserved), ends the state, and shows a death screen back at the estate. Resume-on-reentry preserves the visit-count map across bag/inventory detours. Phase 4 replaces the autobattle stub; passive timed expeditions land in 3.3; mode exclusivity in 3.4.
...
**Not started:** Real combat UI (Phase 4 replaces the exploration autobattle stub), artifact activation, crafting, pets, guilds, arena, market, territorial warfare, passive timed expeditions (3.3) with 2h/day budget, mode exclusivity (3.4). Hunger drain is wired: walk-room drain + combat-round drain + per-room starvation HP tick fire inside `ExplorationService.rollStep` on every step (outward and returning).
