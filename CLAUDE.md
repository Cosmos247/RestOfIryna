...
- `Swift/Models/ExplorationState.swift` — Fluent model, one row per active expedition (user_id unique, stepsDeep = current km). Presence = exploring; absence = at estate. begin/current/end helpers.
...
- `Swift/Migrations/` — CreateUser, AddCharacterFields, AddProfileStyle, AddGameStats, CreateInventory, RemoveCrownsField, AddEquipSlotToInventory, AddGearBonuses, CreateWarehouse, CreateExplorationState
- `Swift/Services/` — Domain services. HungerService is pure (callers persist). EquipmentService owns the atomic slot swap and saves entries + user itself. WarehouseService transfers one unit at a time between inventory and warehouse (skips equipped rows on deposit; preflights inventory space on withdraw and returns a typed result enum). ExplorationService drives a single forward step: hunger drain + starvation HP tick + weighted event roll + autobattle stub for encounters, returning a StepOutcome enum. Currently: HungerService, EquipmentService, WarehouseService, ExplorationService.
...
- `Localizations/` — `en.json`, `uk.json` (~130 keys each)
...
**Exploration (Phase 3.1 active-mode MVP landed):** Tapping 🗺 Explore enters `ExplorationController`. A dedicated reply keyboard `[🚶 Step] [🎒 Bag] [🔙 Return]` replaces main nav for the duration of the expedition. Each step increments `stepsDeep` in the `ExplorationState` row, fires `ExplorationService.rollStep` — weighted event (nothing 40 / loot 30 / encounter 25 / trip 5) at the current depth, plus walk-room hunger drain and starvation HP tick when hunger == 0 — and posts a narrative + status card (depth km, HP/MaxHP, hunger/MaxHunger, starving indicator). Loot is added directly to inventory with a full-bag fallback ("you see X but the bag is full"). Encounters roll an enemy from `EnemyCatalog.pickFor(kmDepth:)` (3 tier-1 enemies currently) and run through a stub autobattle (alternating strikes, ±10% variance, hunger drained per round, safety cap 50); win → loot drops per enemy loot table, lose → HP hits 0. Bag button shows a scoped inline view of food+potions only, with one-tap Eat/Use that refreshes in place — non-consumables are intentionally managed back at the estate. Return button ends the `ExplorationState` and drops back to main with a farewell line. Death wipes every non-equipped `InventoryEntry` row, respawns the player at HP=1 (hunger preserved), ends the state, and shows a death screen back at the estate. Resume-on-reentry: tapping Explore again detects the existing state row and picks up at the same depth — crucial for the bag→inventory→back-to-main→Explore cycle. Phase 4 will replace the autobattle stub with a real round-based combat UI; passive timed expeditions land in Phase 3.3.
...
**Not started:** Real combat UI (Phase 4 replaces the exploration autobattle stub), artifact activation, crafting, pets, guilds, arena, market, territorial warfare, passive timed expeditions (3.3) with 2h/day budget, return-path visited-rooms memory (3.2), mode exclusivity (3.4). Hunger drain is now wired: walk-room drain + combat-round drain + per-room starvation HP tick fire inside `ExplorationService.rollStep`.
