...
2. **Controllers** — each encapsulates a discrete interaction flow. Current and planned controllers:
   ...
   - `EstateController` — tree nav: Root (per-level artwork, tiers 1–3 drawn) → House (Workshop / Kitchen stubs; Warehouse with real deposit/withdraw via `WarehouseService` — stackable types aggregate per item_id with counts on both sides, gear renders per physical row with a single-direction arrow) / Plot stub. Successful deposits/withdraws show an inline `✅ ...` status line above the refreshed warehouse category; failures (nothing to move, backpack full) show a modal alert. Estate level is derived from `user.level` — every 5 player levels bumps it by one.
...
│   │   └── Enemy.swift           # code-based bestiary (EnemyCatalog) — 7 animals across 6 tiers (wild + rabid families); reference doc at content/bestiary.md
│   │
│   ├── Migrations/
│   │   ├── CreateUser.swift
