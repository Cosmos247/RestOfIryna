...
2. **Controllers** — each encapsulates a discrete interaction flow. Current and planned controllers:
   ...
   - `CapitalController` — Phase 6.0 + 6.1 + 6.2 + 6.4. Travel-gated city hub reached from the estate via a 2-minute trip handled by `TravelService` (in-memory `TravelState` row, `Task.detached` timer, `User.location` flips on arrival, `rescheduleInflight` survives bot restarts). Reply-keyboard with 6 location buttons (Market / PvP Arena / Trader / Fortune Teller / Master / Tavern) + utility row [Inventory] [Profile] + 🏡 Back-to-estate. Welcome screen sent as photo (`Assets/capital/welcome.jpg`) + atmospheric lore caption when the trip arrives. Estate / Capital / Explore taps during travel show a countdown banner; taps from the capital onto Explore are blocked ("wilderness only borders the estate"); Estate-tap from capital starts the return trip. **Trader (Phase 6.1)** is live: two-step UX (Menu → Buy/Sell list, edit-in-place over the merchant photo `Assets/capital/trader.jpg`), 11 listings from `TraderCatalog` priced per-unit at 1g/2g/3g/5g/10g/100g sell with flat 2× sell:buy spread, per-row info-modal + `[💸/💰 ×1] [✏️ N]` action buttons. The `[✏️ N]` button opens a "How many?" prompt routed via `EphemeralChatState.PendingTraderTransfer` + `unmatched` text intercept. **Tavern (Phase 6.2)**: button-driven Menu / Dice / Darts; menu sells all 7 cooked dishes (20-200g) bypassing recipe scrolls; gambling uses `bot.sendDice` (returns 1-6 server-side, Telegram animates client-side ~3s) with text labels between pairs since Telegram doesn't let bots author messages as the player. Wager flow: tap stake → "Готовий?" + `[🎲 Кинути][❌ Скасувати]` (no gold debited yet — free cancel) → tap Roll debits + runs the sequence: player label + N dice (2 for dice, 1 for darts) → sleep 4s → house label + N dice → sleep 4s → result message with `[🔄 Зіграти ще раз][🔙 До шинка]`. **Fortune Teller (Phase 6.4)**: tarot draws of the 22 Major Arcana for a 6-hour buff. Tap pays 10g, draws one card uniformly, applies effects (stat additive bonuses + xp/loot/vigor-drain multipliers + one-shot gold/HP/Vigor/XP / Wheel-style 50/50 random), shows the card art (`Assets/capital/fortune/<id>.png`) with name + lore meaning + buff description + 6h countdown. Cooldown between draws is 24h (split from buff window via separate `lastFortuneDrawAt` field). UI handles 3 states: can-draw / cooldown-with-buff (active card + dual timer) / cooldown-only. Active fortune line surfaces in profile (`🔮 <Card> · HH:MM`) across all 3 profile styles. Effects compose into User.effectiveAttack/Defense/Crit/Dodge/Accuracy + User.grantXP + ExplorationService.rollStep (loot weight) + VigorService.drain (drain multiplier). Inline `[🔙 До столиці]` button always present on entry + reveal — also serves as reply-keyboard insurance (tap calls `showCapital` → `sendWelcome` which re-attaches the keyboard if a Telegram client collapsed it after a long inline chain). Same insurance back-button added to tavern + trader menu screens. `onCallbackQuery` forwards unknown callbacks (`pstyle:`, stale `explore:` / `combat:`) to `MainController.onCallbackQuery` instead of returning false — quiet handling instead of "Unsupported content type." Other 3 stubs (Market / Arena / Master) auto-load `Assets/capital/<id>.jpg` if present via `renderLocation`.
