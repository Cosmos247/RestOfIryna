...
│   │   ├── CapitalController.swift       # Phase 6.0–6.5 — travel-gated hub; Trader (buy/sell + ✏️ N) + Tavern (food + dice/darts) + Fortune Teller + Master (armor buy/repair/enchant); 2 stubs left (Market/Arena)
...
│   │   ├── InventoryEntry.swift  # per-user item stacks in the backpack (DB) + helpers. Phase 6.5 added armor `durability`/`max_durability` (start 30) + `enchant_level` (cap 3) columns
...
│   │   └── MasterCatalog.swift   # Phase 6.5 — the Master's shop economics (armor buy prices ≈2.5× resource value, repair cost formula, enchant steps + cap 3). Pure data; durability runtime lives in GearConditionService.
│   │
│   ├── Migrations/
│   │   ├── CreateUser.swift
...
│   │   └── AddGearCondition.swift            # 2026-05-21 — armor `durability` + `max_durability` (default 30) + `enchant_level` (default 0) on inventory. Backs the Master's repair/enchant + combat wear (armor only).
│   │
...
│   │   ├── GearConditionService.swift # Phase 6.5 — armor durability runtime. `drainEquippedArmor(amount:)` spends a fight's wear budget point-by-point across random equipped armor (model C); `WearEvent` victory 1 / defeat 3 / flee 5; `maxDurabilityStart` 30; recomputes bonuses (broken at 0). Armor-only.
│   │   ├── MasterService.swift     # Phase 6.5 — the Master's actions: buy(itemId) / repair(entryId) / enchant(entryId), each draining User.silver (+ materials/durability). Typed result enums; mirrors TraderService/WeaponUpgradeService.
