...
│   │   ├── EstateController.swift        # The estate: Root → House (Workshop · Kitchen · upgrades) / plots, warehouse, per-level artwork. **2026-09-17**: claiming a slot is a two-step now — `estate:plot:type:` draws the picked type's card and asks, `estate:plot:build:` is the only writer (nothing in the code can undo a claim), and every «what it costs / what you have» line on its screens renders through `RequirementLine`
│   │   ├── CapitalController.swift       # Phase 6.0–6.5 — travel-gated hub, split across two streets since 2026-09-20 (👑 Замкова: Базар / Ристалище / Гільдії · 🏘 Підзамче: Крамар / Майстер / Шинок / Ворожка · the square holds only the two roads); Trader (buy/sell + ✏️ N) + Tavern (food + dice/darts) + Fortune Teller + Master (armor buy/repair/enchant) + Market/Базар (player-to-player listings + 🤝 Trade); 1 stub left (Arena)
...
│   │   ├── Item.swift            # Item/GearStats types + ItemCatalog façade + ItemDisplay (roster in content/data/items.json; `EquipmentSlot` moved to ROIContent in rebalance Phase 8) — Phase 5.2 added the Forester's leather set; Phase 5.2.1 added the cooked dishes + `Item.teachesRecipe`; nine dishes ship now — three starters, six taught by the innkeeper's `unlocks` ladder — and the five recipe scrolls were deleted 2026-09-18 because nothing ever granted one
...
│   │   ├── TavernCatalog.swift   # Phase 6.2 — 9 cooked-dish prices (25–180🪙, a flat 2.5× of material cost since 2026-09-18), shared wager tiers [10, 25, 50] for dice + darts
...
│   │   ├── MasterCatalog.swift   # Phase 6.5 — the Master's shop economics (premium armor buy prices ≈4× material value, armor repair formula, weapon repair 1🪙/point, enchant steps + cap 5, `enchantMultiplier` = `1 + enchantBudgetFractionPerLevel × level` — 4% a level of the item's own stats since Phase 6, replacing a flat point curve). Pure data; durability runtime lives in GearConditionService.
...
│   │   ├── AddCapitalStreet.swift  # 2026-09-20 — nullable `capital_street` on `users`: which of the capital's two streets the player's keyboard is showing, nil for the square. Nullable so every existing row starts on the square, which is the screen they already had. A column rather than actor state because `generateControllerKB` is synchronous — `currentKeyboard(for:)` calls it on the refusal path with nowhere to await
...
│   │   ├── ExplorationService.swift # step outcome roll + autobattle stub + loot drops. Also the one place a walked km is TALLIED (`user.recordStep()`) — every kind of step comes through here. The depth RECORD is not written here: it is banked at the manor door from `ExplorationState.maxDepthKm` (2026-09-15)
│   │   ├── LeaderboardService.swift # 2026-09-12 — the four all-time boards behind the journal (level · arena honor · deepest km · total km). One query for the page; a second COUNT only when the viewer is not already on it (a third on the honor board, to find their profile first). Ties share a place; the secondary sort only orders the display. NOT "ratings" — that word means crit/dodge/accuracy here
...
│   │   ├── GearConditionService.swift # Phase 6.5 — gear durability runtime. **2026-09-17**: `drainEquippedGear` / `wear` return `[BrokenPiece]` (item id + the tier of the ROW that broke) rather than `[String]`, because an id cannot name a laddered weapon. `drainEquippedGear(amount:)` spends a fight's wear budget point-by-point across random equipped armor + weapon (model C, shared pool via `durableSlots`); `WearEvent` victory 1 / defeat 3 / flee 5; armor `maxDurabilityStart` 30. `backfillWeaponDurability(on:)` is an idempotent startup pass lifting pre-existing weapons to their per-tier ceiling. At 0: armor broken (0 stats), weapon keeps half (in EquipmentService).
...
│   │   ├── Countdown.swift           # 2026-09-09 — the one "time left" format: `2год 5хв` · `1хв 22сек` · `5хв` · `42сек` (two most significant units; 2026-09-11 extended that rule to minutes+seconds, which had printed minutes alone). Replaced an MM:SS / HH:MM pair no screen could tell apart; every countdown and every authored duration now prints from here
