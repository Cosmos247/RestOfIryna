...
|---|---|
| working tree | clean |
| HEAD | **`065bc6e`** — the King's decrees, content (`978eef7`) then playable (`520513e`: the palace, the engine and all seven event hooks), plus two locale renames and the hash fills. **A commit cannot carry its own hash**, so the newest entry here always trails by one; read HEAD off the machine |
