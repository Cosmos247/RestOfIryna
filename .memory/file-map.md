...
│
├── Localizations/
│   ├── en.json                     # English strings (~334 keys)
│   └── uk.json                     # Ukrainian strings (~334 keys)
│
├── Assets/
...
└── Swift/                          # All source code (SPM target root)
    ...
    │   ├── Item.swift              # Static item catalog: ItemType, ItemEffect, EquipmentSlot (8), GearStats, Item, ItemCatalog (code-based). Phase 5.2: Forester's leather set (hood / jerkin / breeches / boots) added; placeholder `gear.leather_vest` retired into `gear.forester_jerkin` via RenameLeatherVest.
    │   ├── Recipe.swift            # Phase 5.2 — static crafting catalog (RecipeCategory enum forge/tannery, RecipeIngredient, RecipeOutput, Recipe, RecipeCatalog). Code-based like ItemCatalog/EnemyCatalog.
