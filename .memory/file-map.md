...
└── Swift/                          # All source code (SPM target root)
    ...
    │   ├── ExplorationController.swift   # Phase 3.1 active exploration: step/bag/return/death flow; reply keyboard [🚶 Step] [🎒 Bag] [🔙 Return]; uses ExplorationState + ExplorationService; `explore:` callbacks for bag view
    ...
    │   ├── ExplorationState.swift  # Fluent model: one row per active expedition (user_id unique, stepsDeep). Presence = "exploring"; begin/current/end helpers.
    ...
    │   └── ExplorationService.swift # Phase 3.1: rollStep (event weights 40/30/25/5), autobattle stub, loot drop rolls, hunger/starvation integration. StepOutcome enum captures every path (nothing/loot/trip/encounterWon/encounterLost/starvationOnly).
    │
    ├── Telegram/
    │   ├── Router/
