...
*Last updated: 2026-04-27 (Combat UX polish: Attack/Defend/Flee converted to inline buttons so the previous reply keyboard stays put; one-line `combat.in_progress` nudge for misclicks; bot-restart greeting now restores the player's controller-specific reply keyboard for registered users instead of forcing a one-time `/start`. Phase 4.2 concept locked in TODO.md — three signature techniques per class with class-specific Flee chances.)*
