...
//  food-purchase path (silver → bag), atomic preflight on both silver AND slot
...
public enum TavernService {
    ...
    public enum BuyDishResult: Sendable {
        case success(itemId: String, silverSpent: Int)
    ...
    /// buy flow — silver + slot capacity validated up front, then a single
    ...
    public static func buyDish(itemId: String, for user: User, on db: any Database) async throws -> BuyDishResult {
        ...
        if user.silver < listing.priceSilver {
            return .notEnoughSilver(have: user.silver, need: listing.priceSilver)
        }
        ...
        user.silver -= listing.priceSilver
        try await user.saveAndCache(in: db)
        try await InventoryEntry.add(itemId, quantity: 1, to: user, on: db)
        return .success(itemId: itemId, silverSpent: listing.priceSilver)
    }
    ...
    /// classification. Caller applies the resulting silver delta on the User.
